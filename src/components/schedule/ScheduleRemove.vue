<template>
    <base-modal>
        <base-dialog :title="'Delete Schedule'" :closeDialog="closeDialog">
            <p>
                Are you sure you want to delete this schedule? All this schedule's passengers will also be removed.
            </p>
            <base-button-container>
                <base-button type="button" @click="removeData">
                    <i class="fas fa-trash"></i>
                    <span>Remove</span>
                </base-button>
                <base-button type="button" @click="closeDialog">
                    <i class="fas fa-times"></i>
                    <span>Cancel</span>
                </base-button>
            </base-button-container>
        </base-dialog>
    </base-modal>
</template>

<script setup>
import useScheduleStore from '../../store/schedule/schedule-store';
import useRemoveData from '../../hooks/remove-data';

const { id } = defineProps(['id']);

const store = useScheduleStore();

const { removeData,closeDialog } = useRemoveData({ store, backlink: '/schedules', payload:id })

</script>

<style scoped></style>